...
public class digits {
	public static void main(String[] args)  
	{
		...
		for (int i = 0 ; i < b2.length(); i++)
		{
			...
			if (b2.charAt(i) == '0')
			{
				tempb2 = (i == b2.length()-1) ? b2.substring(0,i) + "1" : b2.substring(0,i)+ "1" + b2.substring(i+1, b2.length());
			}
			else
			{
				tempb2 = (i == b2.length()-1) ? b2.substring(0,i) + "0" : b2.substring(0,i)+ "0" + b2.substring(i+1, b2.length());
			}
			...
			for (int k = 0; k < b3.length(); k++)
			{
				...
				for (int j = 0; j <=2 ; j++)
				{
					...
					{
						tempb3 = (k == b3.length()-1) ? b3.substring(0,k) + Integer.toString(j) : b3.substring(0,k)+ Integer.toString(j) + b3.substring(k+1, b3.length());
